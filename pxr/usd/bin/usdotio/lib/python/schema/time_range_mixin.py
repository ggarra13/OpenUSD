    ...
    def _set_time_range(self, stage, usd_path, name):
        ...
        if json_data:
            ...
            if Options.log_level == LogLevel.DEBUG:
                print('\t\tCreated time range at',range_path,'.')

        return range_prim
    
    def _create_time_range(self, usd_prim):
        """Create a TimeRange from a USD GPrim.

        Args:
        usd_prim (GPrim): A valid USD OtioTimeRange GPrim.

        Returns:
        dict:  JSON dict representing the OpenTimelineIO TimeRange.
        """
